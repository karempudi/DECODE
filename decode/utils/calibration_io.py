...
class SplineCoefficient:
    """Wrapper class as an interface for Python Spline calibration data."""
    def __init__(self, calib_file):
        """
        Loads a calibration file from SMAP and the relevant meta information
        Args:
        ...
        """
        self.calib_file = calib_file
        with open(self.calib_file, 'rb') as fh:
            self.calib_dict = pickle.load(fh)

        self.coeff = torch.from_numpy(self.calib_dict['coeff'])
        self.ref0 = (self.calib_dict['x0'], self.calib_dict['x0'], self.calib_dict['z0'])
        self.dz = self.calib_dict['dz']
        self.spline_roi_shape = self.coeff.shape[:3]
